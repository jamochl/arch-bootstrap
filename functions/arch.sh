...
fn_install_pkglists() {
    ...
    if grep '^\w' $PACKAGE_DIR/arch_group_desired.list; then
        yes y | sudo pacman  --needed -S $(cat $PACKAGE_DIR/{common_desired,arch_desired}.list | grep '^\w') $(pacman -Sgq $(grep '^\w' $PACKAGE_DIR/arch_group_desired.list))
    else
        yes y | sudo pacman  --needed -S $(cat $PACKAGE_DIR/{common_desired,arch_desired}.list | grep '^\w')
    fi
